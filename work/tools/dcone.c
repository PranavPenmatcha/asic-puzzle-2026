...
/* --- full transitive cone (for finding WHICH flops matter to `success`) --- */
static int tvisited[NINS];
static int tflops[NINS]; static int ntflops = 0;
static void twalk(const char *net) {
    int d = finddrv(net);
    if (d < 0 || tvisited[d]) return;
    tvisited[d] = 1;
    if (isseq(ins[d].type)) tflops[ntflops++] = d;
    for (int k = 0; k < ins[d].nc; k++) if (!isoutpin(ins[d].c[k].pin)) twalk(ins[d].c[k].net);
}

/* --- per-flop D-cone shape: stops recursion AT flop boundaries --- */
static int dvisited[NINS]; static int dncell;
static int dsrc[NINS]; static int ndsrc;
static void dwalk(const char *net) {
    int d = finddrv(net);
    if (d < 0) return;
    if (isseq(ins[d].type)) {
        for (int i = 0; i < ndsrc; i++) if (dsrc[i] == d) return;
        dsrc[ndsrc++] = d;
        return;
    }
    if (dvisited[d]) return;
    dvisited[d] = 1;
    dncell++;
    for (int k = 0; k < ins[d].nc; k++) if (!isoutpin(ins[d].c[k].pin)) dwalk(ins[d].c[k].net);
}

typedef struct { char name[NM]; int ncell, nsrc; } Row;
static Row rows[NINS]; static int nrows = 0;

static int cmprow(const void *a, const void *b) {
    return ((Row *)a)->nsrc - ((Row *)b)->nsrc;
}
...
int main(int argc, char **argv) {
    ...
    twalk(argv[2]);
    fprintf(stderr, "%s cone: %d flops\n", argv[2], ntflops);

    for (int f = 0; f < ntflops; f++) {
        int fi = tflops[f];
        char *dnet = NULL;
        for (int k = 0; k < ins[fi].nc; k++) if (!strcmp(ins[fi].c[k].pin, "D")) dnet = ins[fi].c[k].net;
        if (!dnet) continue;
        memset(dvisited, 0, sizeof dvisited); dncell = 0; ndsrc = 0;
        dwalk(dnet);
        snprintf(rows[nrows].name, NM, "%s", ins[fi].name);
        rows[nrows].ncell = dncell; rows[nrows].nsrc = ndsrc;
        nrows++;
    }

    qsort(rows, (size_t)nrows, sizeof(Row), cmprow);
    for (int i = 0; i < nrows; i++)
        printf("%-32s cells=%4d srcflops=%3d\n", rows[i].name, rows[i].ncell, rows[i].nsrc);
    return 0;
}
